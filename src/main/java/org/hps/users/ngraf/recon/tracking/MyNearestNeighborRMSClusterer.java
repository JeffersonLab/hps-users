...
public class MyNearestNeighborRMSClusterer implements ClusteringAlgorithm {
     ...
     */
    public MyNearestNeighborRMSClusterer(double seed_threshold, double neighbor_threshold, double cluster_threshold) {
     ...
     */
    public MyNearestNeighborRMSClusterer() {
        this(4.0, 3.0, 4.0);
    }
    ...
    public List<List<LCRelation>> findClusters(List<LCRelation> fittedHits) {
        //System.out.println("Clustering... " + fittedHits.size() + " fitted hits");
...
//        System.out.println("found " + cluster_seeds.size() + " cluster seeds and " + clusterableSet.size() + " possible neighbors ");
...
//            System.out.println("creating cluster with seed channel " + seed_channel);
            ...
            while (unchecked.size() > 0) {
                ...
                cluster_seed_time = FittedRawTrackerHit.getT0(hit);
                Collection<Integer> neighbor_channels = getNearestNeighborCells(clustered_cell);
//                System.out.println("clustered_cell " + clustered_cell + " and time " + cluster_seed_time);
//                System.out.println("has " + neighbor_channels.size() + " neighbors");
...
//                        System.out.println("is not in clusterableSet");
                        continue;
                    }
...
//                        continue;
//                    }
                    // use absolute fitted time difference
//                    System.out.println("delta time " + (FittedRawTrackerHit.getT0(neighbor_hit) - cluster_seed_time));
                    if (Math.abs(FittedRawTrackerHit.getT0(neighbor_hit) - cluster_seed_time) > _neighborDeltaT) {
...
//            System.out.println("cluster has " + cluster.size() + " strips");
