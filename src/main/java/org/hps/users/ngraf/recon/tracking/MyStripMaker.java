...
import org.hps.recon.tracking.ClusteringAlgorithm;
...
public class MyStripMaker {
    ...
    public MyStripMaker(ClusteringAlgorithm algo) {
        _clustering = algo;
    }

    public MyStripMaker(SiSensorSim simulation, ClusteringAlgorithm algo) {
        _clustering = algo;
    ...
    public List<SiTrackerHit> makeHits(SiSensor sensor) {
        ...
        for (LCRelation fittedHit : fittedHits) {
            ...
            if (electrodes instanceof ThinSiStrips) {
                ...
                if ((strip == (HPSSVTConstants.TOTAL_STRIPS_PER_THIN_SENSOR - 1)) || strip == 0) {
                    continue;
                }
                ...
                if (_debug) {
                    System.out.println("StripMaker::makeHits : Electrodes are of SiStriplets");
                }

                if (stripletHits.get(electrodes) == null) {
                    stripletHits.put(electrodes, new ArrayList<LCRelation>());
                }
                ...
                if (strip == HPSSVTConstants.TOTAL_STRIPS_PER_SENSOR) {
                    continue;
                }
    ...
    public List<SiTrackerHit> makeHits(SiSensor sensor, SiSensorElectrodes electrodes, List<LCRelation> fittedHits) {
        ...
        if (_debug) {
            System.out.println(this.getClass().getSimpleName() + "::makeHits : Found clusters = " + cluster_list.size() + " in " + sensor.getName());
        }
        // Create an empty list for the pixel hits to be formed from clusters
        List<SiTrackerHit> hits = new ArrayList<SiTrackerHit>();

        // Make a pixel hit from this cluster
        for (List<LCRelation> cluster : cluster_list) // Make a TrackerHit from the cluster if it meets max cluster size requirement
        {
    ...
    private SiTrackerHitStrip1D makeTrackerHit(List<LCRelation> cluster, SiSensorElectrodes electrodes) {
        ...
        if (_debug) {
            System.out.println("StripMaker::makeTrackerHit : Cluster position: " + position.toString());
        }

        SymmetricMatrix covariance = getCovariance(cluster, electrodes);
        if (_debug) {
            System.out.println("StripMaker::makeTrackerHit : Cluster covariance: " + covariance.toString());
        }
...
//            System.out.println("adding RawTrackerHit channel " + FittedRawTrackerHit.getRawTrackerHit(bth).getIdentifierFieldValue("strip") + " " + FittedRawTrackerHit.getAmp(bth) + " " + FittedRawTrackerHit.getT0(bth));
//            System.out.println("adding RawTrackerHit time " + FittedRawTrackerHit.getRawTrackerHit(bth).getTime());
    ...
    private Hep3Vector getPosition(List<LCRelation> cluster, SiSensorElectrodes electrodes) {
        if (_debug) {
            System.out.println("StripMaker::getPosition : Calculating position for cluster size " + cluster.size());
        }
        ...
        for (LCRelation hit : cluster) {
            ...
            int channel = FittedRawTrackerHit.getRawTrackerHit(hit).getIdentifierFieldValue("strip");
//            System.out.println("channel: " + channel + " amp: " + FittedRawTrackerHit.getAmp(hit) + " time: " + FittedRawTrackerHit.getT0(hit));
            if (electrodes instanceof ThinSiStrips) {
...
//                    System.out.println("thinStripCenter is at " + ((ThinSiStrips) electrodes).getStripCenter(stripMap_.get(hit)).toString());
                ...
                if (_debug) {
                    System.out.println("StripMaker::getPosition : Position " + position.toString());
                }
