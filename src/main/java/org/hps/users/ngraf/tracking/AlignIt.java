...
//        }
        ...
        int[][] MASK = new int[6][NN];
        GEN_OFFSETS(NN, MASK);

//        aida.saveAs("AlignIt.aida");
    }

    static void GEN_OFFSETS(int NN, int[][] MASK) {//(, double[][] DANG, double[][] DUVW, double[][] R0, double[][] R0C, double[][] ROT, double[][] ROTC) {
//*DOC  Input:
...
//*DOC          ROTC = new rotation      
        ...
        // check to see which planes have been altered and set MASK accordingly
        for (int i = 0; i < NN; ++i) {
            for (int j = 0; j < 3; ++j) {
                if (dq.get(i)[j] != 0.) {
                    MASK[j + 3][i] = 1;  // displaced
                }
                if (da.get(i)[j] != 0.) {
                    MASK[j + 3][i] = 1; //rotated
                }
            }
        }
