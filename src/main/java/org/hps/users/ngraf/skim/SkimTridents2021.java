 ...
 */
public class SkimTridents2021 extends Driver {
...
//        List<ReconstructedParticle> V0List = event.get(ReconstructedParticle.class, "UnconstrainedV0Candidates_KF");
//        int nV0 = V0List.size();
//        List<ReconstructedParticle> VCList = event.get(ReconstructedParticle.class, "UnconstrainedVcCandidates_KF");
//        int nVc = VCList.size();
...
//                if (rpList.size() == 3) {
            ...
            for (ReconstructedParticle rp : rpList) {
                if (!rp.getParticleIDs().isEmpty()) {
                    ...
                    if (pdgId == 11) {
                        if (isKF && !rp.getClusters().isEmpty()) {// && rp.getTracks().get(0).getTrackerHits().size() >= 13) {
                            electrons.add(rp);
                        }
                    }
                    if (pdgId == -11) {
            ...
            if (electrons.size() == 2 && positrons.size() == 1) {
                ...
                if (!positrons.get(0).getClusters().isEmpty()) {
                    ...
                    aida.histogram1D("tridentvec px", 100, -1., 1.).fill(tridentVec.x());
                    aida.histogram1D("tridentvec py", 100, -1., 1.).fill(tridentVec.y());
                    aida.histogram1D("tridentvec pz", 100, 0., 10.).fill(tridentVec.z());
                    aida.histogram1D("tridentvec E", 100, 0., 10.).fill(tridentVec.magnitude());
                    ...
                    // before cuts
                    aida.histogram1D("delta track time ele1 ele2 before time cuts", 100, -50., 50.).fill(ele1TrackTime - ele2TrackTime);
                    aida.histogram1D("delta track time ele1 pos1 before time cuts", 100, -50., 50.).fill(ele1TrackTime - posTrackTime);
                    aida.histogram1D("delta track time ele2 pos1 before time cuts", 100, -50., 50.).fill(ele2TrackTime - posTrackTime);
                    ...
                    if (abs(dt1) < trackDeltaTimeCut && abs(dt2) < trackDeltaTimeCut && abs(dt3) < trackDeltaTimeCut) {
                        ...
                        if (abs(dc1) < clusterDeltaTimeCut && abs(dc2) < clusterDeltaTimeCut && abs(dc3) < clusterDeltaTimeCut) {
                            ...
                            aida.histogram1D("tridentvec px final", 100, -1., 1.).fill(tridentVec.x());
                            aida.histogram1D("tridentvec py final", 100, -1., 1.).fill(tridentVec.y());
                            aida.histogram1D("tridentvec pz final", 100, 0., 10.).fill(tridentVec.z());
                            aida.histogram1D("tridentvec E final", 100, 0., 10.).fill(tridentVec.magnitude());
