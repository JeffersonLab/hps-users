...
public class SkimEcalFeeWabTrident2019 extends Driver {
    ...
    private double _maxTridentClusterEnergy = 3.0;
...
//                            -90.0, 90.0).fill(pos2.x(), pos2.y());
        ...
        if (ecalClusters.size() > 2) {
            ...
            for (Cluster cluster : ecalClusters) {
                ...
                if (cluster.getEnergy() > _minTridentClusterEnergy && cluster.getEnergy() < _maxTridentClusterEnergy) {
                    if (cluster.getPosition()[0] > 100.) {
                        aida.histogram1D("all cluster positron energy", 100, 0., 7.).fill(cluster.getEnergy());
                        positrons.add(cluster);
                    }
                    if (cluster.getPosition()[0] < 0.) {
                        aida.histogram1D("all cluster electron energy", 100, 0., 7.).fill(cluster.getEnergy());
            ...
            if (positrons.size() > 0 && electrons.size() > 1) {
                ...
                if (tridentClusters.size() == 3) {
                    ...
                    aida.histogram1D("trident positron energy", 100, 0., 7.).fill(positron.getEnergy());
                    aida.histogram1D("trident electron energy 1", 100, 0., 7.).fill(electron1.getEnergy());
                    aida.histogram1D("trident electron energy 2", 100, 0., 7.).fill(electron2.getEnergy());
                    aida.histogram1D("trident esum", 100, 0., 7.).fill(esum);
                    ...
                    if (TriggerModule.inFiducialRegion(positron) && TriggerModule.inFiducialRegion(electron1) && TriggerModule.inFiducialRegion(electron2)) {
                        aida.histogram1D("trident positron energy all fiducial", 100, 0., 7.).fill(positron.getEnergy());
                        aida.histogram1D("trident electron energy 1 all fiducial", 100, 0., 7.).fill(electron1.getEnergy());
                        aida.histogram1D("trident electron energy 2 all fiducial", 100, 0., 7.).fill(electron2.getEnergy());
                        aida.histogram1D("trident esum all fiducial", 100, 0., 7.).fill(esum);
    ...
    private void analyzeTridentEvent(EventHeader event, List<Cluster> ecalClusters) {
        ...
        if (event.hasCollection(ReconstructedParticle.class, "FinalStateParticles_KF")) {
            ...
            aida.histogram1D("cluster esum all events", 100, 0., 7.).fill(esum);
            for (int i = 0; i < 3; ++i) {
            ...
            if (positron != null && electron1 != null && electron2 != null) {
                ...
                aida.histogram1D("cluster esum all matched to RP", 100, 0., 7.).fill(esum);
                aida.histogram1D("trident positron cluster energy", 100, 0., 7.).fill(positron.getEnergy());
                aida.histogram1D("trident electron1 cluster energy", 100, 0., 7.).fill(electron1.getEnergy());
                aida.histogram1D("trident electron2 cluster energy", 100, 0., 7.).fill(electron2.getEnergy());
                if (positronIsPositron) {
                    aida.histogram1D("trident positron RP energy", 100, 0., 7.).fill(positron.getEnergy());
                }
                if (electron1IsElectron) {
                    aida.histogram1D("trident electron1 RP energy", 100, 0., 7.).fill(electron1.getEnergy());
                }
                if (electron2IsElectron) {
                    aida.histogram1D("trident electron2 RP energy", 100, 0., 7.).fill(electron2.getEnergy());
                }
                ...
                if (positronIsPositron && electron1IsElectron && electron2IsElectron) {
                    ...
                    aida.histogram1D("cluster esum all matched to correct sign track", 100, 0., 7.).fill(esum);
                    ...
                    aida.histogram1D("trident energy rot " + _beamAxisRotationY, 100, 0.0, 10.).fill(trident.t());
