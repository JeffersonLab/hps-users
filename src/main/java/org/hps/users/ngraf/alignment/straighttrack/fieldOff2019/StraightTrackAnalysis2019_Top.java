package org.hps.users.ngraf.alignment.straighttrack.fieldOff2019;
...
//            System.out.println("module " + moduleName);
            if (moduleName.contains("axial") && moduleName.contains("t_half")) {
                ...
                if (layerInt > 2) {
                    ...
                    aida.histogram1D(layer + " measured", 50, 0., 50.).fill(stripClusterPos[1]);
                    aida.histogram1D(layer + " predicted", 50, 0., 50.).fill(predicted);
                    double dPos = stripClusterPos[1] - predicted;
                    aida.histogram1D(layer + " measured - predicted", 50, -40., 10.).fill(stripClusterPos[1] - predicted);

                    if (hm.get(moduleName) < nMaxHits) {
                        aida.histogram1D(layer + " < " + nMaxHits + " hits hit measured", 50, 0., 50.).fill(stripClusterPos[1]);
                        aida.histogram1D(layer + " < " + nMaxHits + " hits hit predicted", 50, 0., 50.).fill(predicted);
                        aida.histogram1D(layer + " < " + nMaxHits + " hits hit measured - predicted", 50, -40., 10.).fill(stripClusterPos[1] - predicted);
...
//                    aida.cloud1D(layer + " " + hm.get(moduleName) + " hit measured - predicted").fill(stripClusterPos[1] - predicted);
        ...
        if (oneHitInEachLayer) {
            ...
            aida.histogram1D("zIntercept56", 100, -4000., -1000.).fill(tpl56.xAxisIntercept());
            ...
            double y56at400 = tpl56.predict(zPos);
            aida.histogram1D("y56at400 - y34at400", 100, -2., 2.).fill(y56at400 - y34at400);

            if (allSingleHits) {
                aida.histogram1D("slope56 - slope34 All Single Hits", 100, -0.005, 0.005).fill(tpl56.slope() - tpl34.slope());
                aida.histogram1D("y56at400 - y34at400 All Single Hits", 100, -2., 2.).fill(y56at400 - y34at400);
            }

            if (allDoubleHits) {
                aida.histogram1D("slope56 - slope34 All Double Hits", 100, -0.005, 0.005).fill(tpl56.slope() - tpl34.slope());
                aida.histogram1D("y56at400 - y34at400 All Double Hits", 100, -2., 2.).fill(y56at400 - y34at400);
            }
        }
