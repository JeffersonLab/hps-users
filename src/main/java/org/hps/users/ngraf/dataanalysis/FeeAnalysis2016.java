 ...
 */
public class FeeAnalysis2016 extends Driver {

    private AIDA aida = AIDA.defaultInstance();
    private final BasicHep3Matrix beamAxisRotation = new BasicHep3Matrix();

    private boolean _analyzeTracksByNhits = true;
    String[] ReconstructedParticleCollectionNames = {"FinalStateParticles_KF", "FinalStateParticles"};//, "OtherElectrons", "OtherElectrons_KF"};

    private boolean _skimFee = true;
    private boolean _isFeeCandidate;
    private double _minEnergy = 1.5;
    ...
    private int _maxClustersPerCrystal = 2000;
...
//                    String otherElectrons = rpCollectionName.contains("_KF") ? "OtherElectrons" : "OtherElectrons_KF";
//                    String otherElectrons = "OtherElectrons";
...
//        aida.cloud1D("Track Type").fill(t.getType());
        ...
        if (nHits > 9 && p < 4.) {
            ...
            aida.histogram1D("track tanlambda" + topOrBottom, 50, 0., 0.1).fill(abs(tanLambda));
            aida.histogram2D("Track tanlambda vs p" + topOrBottom, 50, 0., 0.1, 100, 0., 4.0).fill(abs(tanLambda), p);
            aida.profile1D("Track tanlambda vs p profile" + topOrBottom, 50, 0., 0.1).fill(abs(tanLambda), p);
