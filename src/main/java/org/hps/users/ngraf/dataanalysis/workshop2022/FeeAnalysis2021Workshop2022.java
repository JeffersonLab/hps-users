...
import org.hps.users.ngraf.dataanalysis.*;
 ...
 */
public class FeeAnalysis2021Workshop2022 extends Driver {
    ...
    protected void process(EventHeader event) {
        ...
        for (Cluster cluster : clusters) {
            ...
            if (isFiducial) {
                ...
                aida.histogram2D("cluster energy vs seed energy", 100, 0., 5., 100, 0., 2.5).fill(clusterEnergy, seedEnergy);
                if (seedEnergy >= _seedHitEnergyFraction * _beamEnergy) {
                    // found a fiducial cluster with large fraction of energy in the seed crystal
                    nFeeClusters++;
                    feeCluster = cluster;
                    int ix = seed.getIdentifierFieldValue("ix");
                    int iy = seed.getIdentifierFieldValue("iy");
                    aida.histogram2D("selected cluster ix vs iy", 47, -23.5, 23.5, 11, -5.5, 5.5).fill(ix, iy);
                    ...
                    if (cluster.getPosition()[1] > 0.) {
                        ...
                        if (nClusters == 1) {
                            aida.histogram1D("Top single cluster energy ", 200, 0.0, 10.0).fill(cluster.getEnergy());
                        }
                    } else {

                        aida.histogram1D("Bottom cluster energy ", 200, 0.0, 10.0).fill(cluster.getEnergy());
                        if (nClusters == 1) {
                            aida.histogram1D("Bottom single cluster energy ", 200, 0.0, 10.0).fill(cluster.getEnergy());
                        }
                    }
                } // end of seed energy fraction check
            } // end of fiducial check
        }

        // only analyze events with a single good FEE cluster
        if (nFeeClusters == 1) {
            // let's find the corresponding ReconstructedParticle...
                ...
                if (event.hasCollection(ReconstructedParticle.class, rpCollectionName)) {
                    ...
                    for (ReconstructedParticle rp : rpList) {
                        ...
                        if (!rp.getClusters().isEmpty()) {
                            //is the cluster the selected FEE cluster?
                            if (rp.getClusters().get(0) == feeCluster) {
                                _fee = rp;
                            }
                        }
                    }
                    // only analyze the FEE candidates
                    if (_fee != null) {
                        setupSensors(event);
                        _trackToData = getKFTrackDataRelations(event);
                        skipEvent = false;
                        analyzeReconstructedParticle(_fee);
                    }
                    aida.tree().cd("..");
                }
            }
...
//        if (rp.getClusters().size() == 1) {
        ...
        aida.histogram2D(type + "Cluster x vs y", 320, -270.0, 370.0, 90, -90.0, 90.0).fill(c.getPosition()[0], c.getPosition()[1]);
        if (c.getPosition()[1] > 0.) {

            aida.histogram1D(type + "Top cluster energy ", 200, 0.0, 10.0).fill(c.getEnergy());
        } else {

            aida.histogram1D(type + "Bottom cluster energy ", 200, 0.0, 10.0).fill(c.getEnergy());
        }
//        }
    }
    ...
    void analyzeTrack(ReconstructedParticle rp) {
        ...
        aida.histogram1D("Track nHits" + topOrBottom + "all tracks", 14, 0.5, 14.5).fill(t.getTrackerHits().size());
        aida.histogram1D("Track momentum" + topOrBottom + "all tracks", 200, 0., 10.0).fill(p);
