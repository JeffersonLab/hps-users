...
public class EventAnalysis2019 extends Driver {
    ...
    String[] clusterCollections = {"EcalClusters", "EcalClustersCorr"};
...
//        if (event.hasCollection(ReconstructedParticle.class, "FinalStateParticles")) {
        ...
        int nFittedTrackerHits = fittedTrackerHits.size();
        ...
        aida.histogram1D("SVT number of FittedTrackerHits", 100, 0., 1000.).fill(nFittedTrackerHits);
        aida.histogram1D("SVT number of StripHits", 100, 0., 1000.).fill(nStripHits);
        aida.histogram2D("SVT number of RawTrackerHits vs FittedTrackerHits", 100, 0., 1000., 100, 0., 1000.).fill(nRawTrackerHits, nFittedTrackerHits);
...
//            }
        ...
        analyzeV0(event);
//        }
        for (String clusterCollection : clusterCollections) {
            if (event.hasCollection(Cluster.class, clusterCollection)) {
                analyzeClusters(event, clusterCollection);
            }
        }
    }
...
//            }
                        ...
                        if (!clusters.isEmpty()) {
                            ...
                            String fid = TriggerModule.inFiducialRegion(c) ? " fiducial " : " ";
                            analyzeCluster(c, id);
                            double[] cPos = c.getPosition();
                            aida.histogram2D(id + " cluster x vs y", 320, -270.0, 370.0, 90, -90.0, 90.0).fill(cPos[0], cPos[1]);
                            aida.histogram1D(trackType + torb + id + fid + "cluster energy", 100, 0., 10.).fill(c.getEnergy());
                            aida.histogram1D(trackType + torb + id + fid + "cluster energy over track momentum EoverP", 100, 0., 2.).fill(c.getEnergy() / rp.getMomentum().magnitude());
                            aida.histogram1D(trackType + torb + id + " track momentum with " + fid + "cluster", 100, 0., 10.).fill(rp.getMomentum().magnitude());
                            aida.histogram1D(trackType + torb + id + " track momentum with " + fid + "cluster " + nHits + " hits", 100, 0., 10.).fill(rp.getMomentum().magnitude());
                            aida.histogram1D(trackType + " " + trackFinderType + torb + id + " track momentum with " + fid + "cluster " + nHits + " hits", 100, 0., 10.).fill(rp.getMomentum().magnitude());
    ...
    private void analyzeClusters(EventHeader event, String clusterCollection) {
