 ...
 */
public class FeeVertexAnalysis extends Driver {
...
//        volumes.add("");
        ...
        for (String vol : volumes) {
            ...
            aida.histogram1D(vtxFolder + "vtx_z" + vol, 100, -10., 10.);
            aida.histogram2D(vtxFolder + "vtx_x_y" + vol, 200, -3, 3, 200, -1, 1);
            aida.histogram2D(vtxFolder + "vtx_x_z" + vol, 100, -3, 3, 100, -10., 10.);
            aida.histogram2D(vtxFolder + "vtx_y_z" + vol, 100, -1, 1, 100, -10., 10.);
...
//TODO add some quality cuts on the track, e.g. nHits, associated cluster, charge, etc.
                if (!rp.getClusters().isEmpty()) {
                    ...
                    //require fiducial cluster
                    if (TriggerModule.inFiducialRegion(rp.getClusters().get(0))) {
                        Cluster c = rp.getClusters().get(0);
                        double eclus = c.getEnergy();
                        CalorimeterHit seed = c.getCalorimeterHits().get(0);
//                        if (seed.getCorrectedEnergy() > 2.7) {
                        if (eclus > 3.2) {
                            aida.histogram1D("seed energy", 100, 0.0, 5.0).fill(seed.getCorrectedEnergy());
...
//            System.out.println("Fitting " + accumulatedBTracksBot.size() + " bottom tracks");
                ...
                aida.histogram2D(vtxFolder + "vtx_x_z" + vol).fill(vtx.getPosition().x(), vtx.getPosition().z());
                aida.histogram2D(vtxFolder + "vtx_y_z" + vol).fill(vtx.getPosition().y(), vtx.getPosition().z());
