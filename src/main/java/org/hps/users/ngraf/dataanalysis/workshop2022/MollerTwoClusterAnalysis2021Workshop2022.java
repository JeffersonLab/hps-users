 ...
 */
public class MollerTwoClusterAnalysis2021Workshop2022 extends Driver {
    ...
    protected void process(EventHeader event) {
        ...
        List<Cluster> mollerFiducialClusters = new ArrayList<>();
        Cluster clus = null;
        ...
        for (Cluster cluster : clusters) {
            ...
            if (iy >= _iyMin && iy <= _iyMax && ix >= _ixMin && ix <= _ixMax) {
                ...
                if (clus.getEnergy() > _clusterEnergyMin) {
                    nFiducialClusters++;
                    mollerFiducialClusters.add(cluster);
        ...
        aida.tree().cd("..");
        //continue only with events that have at least two clusters in the Moller trigger fiducial region
        if (mollerFiducialClusters.size() >= 2) {
            //let's find out how many of these clusters have a track associated with them...
            for (String rpCollectionName : ReconstructedParticleCollectionNames) {
                aida.tree().mkdirs("Moller two cluster analysis " + rpCollectionName);
                aida.tree().cd("Moller two cluster analysis " + rpCollectionName);
                List<ReconstructedParticle> electronsWithClusters = new ArrayList<>();
                List<ReconstructedParticle> rpList = event.get(ReconstructedParticle.class, rpCollectionName);
                // loop over all RPs in event
                for (ReconstructedParticle rp : rpList) {
                    int pdgId = rp.getParticleIDUsed().getPDG();
                    // looking only for electrons...
                    if (pdgId == 11) {
                        // let's see if this electron is associated with a cluster of interest that triggered...
                        if (!rp.getClusters().isEmpty()) {
                            if (mollerFiducialClusters.contains(rp.getClusters().get(0))) {
                                electronsWithClusters.add(rp);
                            }
                        }
                    }
                }
                aida.histogram1D("number of Moller Fiducial Clusters", 5, -0.5, 4.5).fill(nFiducialClusters);
                aida.histogram1D("number of electrons with Moller Fiducial Clusters", 5, -0.5, 4.5).fill(electronsWithClusters.size());
                //let's start with events that have two and only two electrons matched to ecal clusters in the trigger fiducial region
                if (electronsWithClusters.size() == 2) {
                    RelationalTable trackToData = getKFTrackDataRelations(event);
                    ReconstructedParticle electron1 = electronsWithClusters.get(0);
                    ReconstructedParticle electron2 = electronsWithClusters.get(1);
                    Track track1 = electron1.getTracks().get(0);
                    Cluster cluster1 = electron1.getClusters().get(0);
                    Track track2 = electron2.getTracks().get(0);
                    Cluster cluster2 = electron2.getClusters().get(0);
                    double clusterTime1 = ClusterUtilities.findSeedHit(cluster1).getTime();
                    double clusterTime2 = ClusterUtilities.findSeedHit(cluster2).getTime();
                    // calorimeter cluster times are well understood and trusted...
                    if (abs(clusterTime1 - clusterTime2) < 2.0) {
                        int nHits1 = track1.getTrackerHits().size();
                        int nHits2 = track2.getTrackerHits().size();
                        // high number of hits on both tracks
                        if (nHits1 >= 13 && nHits2 >= 13) {
                            double trackTime1 = ((GenericObject) trackToData.from(track1)).getFloatVal(0);
                            double trackTime2 = ((GenericObject) trackToData.from(track2)).getFloatVal(0);
                            double p1 = electron1.getMomentum().magnitude();
                            double p2 = electron2.getMomentum().magnitude();
                            boolean e1IsTop = electron1.getMomentum().y() > 0 ? true : false;
                            boolean e2IsTop = electron2.getMomentum().y() > 0 ? true : false;
                            // cuts on individual track momenta
                            if (p2 > _track2MinimumMomentum && p2 < _track2MaximumMomentum) {
                                if (p1 > _track2MinimumMomentum && p1 < _track1MaximumMomentum) {
                                    // cut on sum pY
                                    if (abs(electron1.getMomentum().y() + electron2.getMomentum().y()) < _maxMomentumYSum) {
                                        double e1 = cluster1.getEnergy();
                                        double e2 = cluster2.getEnergy();
                                        double sumPx = electron1.getMomentum().x() + electron2.getMomentum().x();
                                        double sumPy = electron1.getMomentum().y() + electron2.getMomentum().y();
                                        double psum = electron1.getMomentum().magnitude() + electron2.getMomentum().magnitude();
                                        double esum = electron1.getEnergy() + electron2.getEnergy();
                                        ...
                                        CalorimeterHit seed2 = cluster2.getCalorimeterHits().get(0);
                                        int ix2 = seed2.getIdentifierFieldValue("ix");
                                        int iy2 = seed2.getIdentifierFieldValue("iy");
                                        aida.histogram2D("cluster 1 ix vs iy", 47, -23.5, 23.5, 11, -5.5, 5.5).fill(ix1, iy1);
                                        aida.histogram2D("cluster 2 ix vs iy", 47, -23.5, 23.5, 11, -5.5, 5.5).fill(ix2, iy2);
                                        aida.histogram1D("track momentum1", 100, 0., 3.0).fill(p1);
                                        aida.histogram1D("track momentum2", 100, 0., 3.0).fill(p2);
                                        aida.histogram1D("cluster energy1", 100, 0., 3.0).fill(e1);
                                        aida.histogram1D("cluster energy2", 100, 0., 3.0).fill(e2);
                                        aida.histogram1D("track momentum1", 100, 0., 3.0).fill(p1);
                                        aida.histogram1D("track momentum2", 100, 0., 3.0).fill(p2);
                                        if (e1IsTop) {
                                            aida.histogram1D("track momentum top", 100, 0., 3.0).fill(p1);
                                        } else {
                                            aida.histogram1D("track momentum bottom", 100, 0., 3.0).fill(p1);
                                        }
                                        if (e2IsTop) {
                                            aida.histogram1D("track momentum top", 100, 0., 3.0).fill(p2);
                                        } else {
                                            aida.histogram1D("track momentum bottom", 100, 0., 3.0).fill(p2);
                                        }
                                        aida.histogram1D("psum", 100, 0., 3.).fill(psum);
                                        aida.histogram1D("esum", 100, 0., 3.).fill(esum);
                                        aida.histogram1D("track sum pX", 100, 0.0, 0.1).fill(sumPx);
                                        aida.histogram1D("track sum pY", 100, -0.05, 0.05).fill(sumPy);

                                        aida.histogram1D("track time - cluster time electron1", 100, -100., 0.).fill(trackTime1 - clusterTime1);
                                        aida.histogram1D("track time - cluster time electron2", 100, -100., 0.).fill(trackTime2 - clusterTime2);
                                        aida.histogram1D("track time 1 - track time 2", 100, -10., 10.).fill(trackTime1 - trackTime2);
                                        aida.histogram1D("cluster time 1 - cluster time 2", 100, -10., 10.).fill(clusterTime1 - clusterTime2);
                                        aida.histogram2D("cluster dt vs track dt", 100, -10., 10., 100, -10., 10.).fill(clusterTime1 - clusterTime2, trackTime1 - trackTime2);
                                        aida.histogram1D("track1 nHits", 20, -0.5, 19.5).fill(nHits1);
                                        aida.histogram1D("track2 nHits", 20, -0.5, 19.5).fill(nHits2);
                                        analyzeTwoElectrons(electron1, electron2);
                                        makeMollerPlots(electron1, electron2);
                                        int nRPs = rpList.size();
                                        if (nClusters < 6) {
                                            aida.histogram1D("track sum pX " + nClusters + " clusters", 100, 0.0, 0.1).fill(sumPx);
                                            aida.histogram1D("track sum pY " + nClusters + " clusters", 100, -0.05, 0.05).fill(sumPy);
                                            aida.histogram1D("psum " + nClusters + " clusters", 100, 0., 3.).fill(psum);
                                            aida.histogram1D("invariant mass " + nClusters + " clusters", 100, 0.0, 0.1).fill(_eemass);
                                        }
                                        if (nRPs < 9) {
                                            aida.histogram1D("track sum pX " + nRPs + " ReconstructedParticles", 100, 0.0, 0.1).fill(sumPx);
                                            aida.histogram1D("track sum pY " + nRPs + " ReconstructedParticles", 100, -0.05, 0.05).fill(sumPy);
                                            aida.histogram1D("psum " + nRPs + " ReconstructedParticles", 100, 0., 3.).fill(psum);
                                            aida.histogram1D("invariant mass " + nRPs + " ReconstructedParticles", 100, 0.0, 0.1).fill(_eemass);
                                        }
                                        // this should be a good event...
                                        _skipEvent = false;
                                        // monitoring plots
                                        aida.histogram1D("number of clusters in event", 10, -0.5, 9.5).fill(nClusters);
                                        aida.histogram1D("number of RPs in event", 10, -0.5, 9.5).fill(rpList.size());
                                    }
                                }
                            }
                        }
...
//        aida.cloud2D("p2rot x vs y").fill(p2rot.x(), p2rot.y());
    ...
    private void analyzeTwoElectrons(ReconstructedParticle ele1, ReconstructedParticle ele2) {
        ...
        _eemass = eesum.mass();
        aida.histogram1D("invariant mass", 100, 0.0, 0.1).fill(_eemass);
    }
