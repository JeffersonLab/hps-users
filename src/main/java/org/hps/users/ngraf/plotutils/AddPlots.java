...
public class AddPlots {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int fileIndex = 1; fileIndex < inputFiles.size(); fileIndex++) {
            ...
            List<String> histoNamesList = new ArrayList<String>();
            String[] newObjectTypes = srcTree.listObjectTypes("/", true);
            String[] newObjectNames = srcTree.listObjectNames("/", true);
            for (int pathIndex = 0; pathIndex < newObjectNames.length; pathIndex++) {
                if (newObjectTypes[pathIndex].startsWith("IHistogram")) {
                    histoNamesList.add(newObjectNames[pathIndex]);
                }
                LOGGER.fine(newObjectNames[pathIndex] + ":" + newObjectTypes[pathIndex]);
            }
